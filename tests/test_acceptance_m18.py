...
def test_m18_pending_count_command():
    """Verify `pending-count` returns the correct count of stored items."""
    # Empty list should return 0
    empty = run_cli("pending-count")
    ...
    result = run_cli("pending-count")
    assert result.returncode == 0, result.stderr + result.stdout
    assert "2" in result.stdout


def test_m18_pending_count_readme_documented():
    """Verify README documents `pending-count`."""
    assert README_FILE.exists()
    readme = README_FILE.read_text(encoding="utf-8").lower()
    assert "pending-count" in readme
