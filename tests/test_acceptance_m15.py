...
def test_m15_item_summary_command():
    """Verify `item-summary` returns the correct count of stored items."""
    # Empty list should return 0
    empty = run_cli("item-summary")
    ...
    result = run_cli("item-summary")
    assert result.returncode == 0, result.stderr + result.stdout
    assert "2" in result.stdout


def test_m15_item_summary_readme_documented():
    """Verify README documents `item-summary`."""
    assert README_FILE.exists()
    readme = README_FILE.read_text(encoding="utf-8").lower()
    assert "item-summary" in readme
